...
co_fut1 = fut |>
    group_by(Date) |>
        mutate(Return = Delt(Close), CO = wilderSum((sign(Return)*Volume*Close),n=2)/runMean(Volume*Close,n=2),
          rsd = rolled_sd(Return), MODCO = CO/rsd)

co_fut2 = fut |>
    group_by(Date) |>
        mutate(Return = Delt(Close), CO = wilderSum((sign(Return)*Volume*Close),n=4)/runMean(Volume*Close,n=4),
          rsd = rolled_sd(Return), MODCO = CO/rsd)
...
combmin5 |>
  ggplot(aes( x = CO, y = Int_Ret)) +
   geom_point(size = 3) + 
     geom_smooth(method = lm) +
       labs( title = "Russell 2000 CO and 5 Minute Return",
       x = "Continuing Overconfidence", y = "Return",
       subtitle = "CO from 7:00 to 9:30 and Returns from 9:30 to 9:35")
