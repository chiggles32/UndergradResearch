install.packages(c("tidyverse","jsonlite","lubridate","quantmod","PerformanceAnalytics", "tibbletime","purrr","ggridges","gtsummary","TTR"))
library(tidyverse)
library(jsonlite)
library(lubridate)
library(quantmod)
...
int_ret = etf |>
      group_by(Date) |>
        mutate(Int_Ret = Delt(Close)) |>
            select(Date, Time, Int_Ret) 

int_ret = int_ret |>
  mutate(GR = case_when( Time < "12:00:00" ~ "M", Time >= "12:00:00" ~ "A"))

std = int_ret |>
  group_by(Date, GR) |>
    summarize(sdval = sd(Int_Ret, na.rm = TRUE))

STD = as.data.frame(std)


std |>
  ggplot( aes(y = GR, x = sdval, fill = GR)) +
